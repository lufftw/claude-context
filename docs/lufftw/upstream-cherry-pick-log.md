...
| `fa93b64` | feat: OpenRouter as embedding provider name (#304) | config.ts (2 markers); embedding.ts (1) — all mechanical union expansion | OK | none | none | jsonrpc/snap/fregr OK | bc1887e; SYNTHESIZE: union expanded to `'OpenAI'|'VoyageAI'|'Gemini'|'Ollama'|'OpenRouter'|'RabbitMQ'` in 3 places; OpenRouter reuses `OpenAIEmbedding` so no new return-type entry |
| `b03ebac` | fix(mcp): subdirectory paths to indexed parent | handlers.ts (2 markers) | n/a | n/a | n/a | build FAIL: TS2339 `queryCollectionStats` not on ToolHandlers | **REVERTED** at 97abd74. b03ebac references `queryCollectionStats` which is added by `ae0fd79` (DEFERRED to Phase A). Hidden dependency not caught by Phase 0.2.3 source-verify. **SKIP-CASCADE: depends on ae0fd79** — added to Phase A list. |
| `a83f260` | fix(mcp): pass embedding dimension to Ollama | config.ts (2 markers); embedding.ts (1 trivial brace) | OK | none | none | build OK | 9b047fe; SYNTHESIZE: ContextMcpConfig interface accepts BOTH `ollamaDimension` AND fork's `rabbitmqUrl/Queue/...`; createMcpConfig populates both blocks; case 'RabbitMQ' arm in switch preserved |
| `968cce6` | fix(mcp): clean up orphan merkle snapshots | handlers.ts (1 marker) | OK | none | none | jsonrpc/snap/fregr OK | e944f6a; SYNTHESIZE: removed dynamic FileSynchronizer import (now top-level); preserved fork's `ignorePatterns` and `includeDotDirs` getter calls; orphan-cleanup `FileSynchronizer.deleteSnapshot()` auto-merged at line 267 |
